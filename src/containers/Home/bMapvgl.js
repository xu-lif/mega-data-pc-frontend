...
const Home = () => {
  ...
  // 生成曲线节点
  const createCurvePoint = (data) => {
    if (data && data.length) {
      return new mapvgl.OdCurve({
        points: data,
      });
    }
  };
    ...
    dataList.forEach((val) => {
      if (val && val.position && val.position.length > 1) {
        ...
        lineLayerData.forEach((valData) => {
          const curveData = createCurvePoint(valData).getPoints();
          console.log("curveData", curveData, valData);
          lineData.push({
            geometry: {
              type: "LineString",
              coordinates: valData,
        ...
        singleLineLayer.setData(lineData);
