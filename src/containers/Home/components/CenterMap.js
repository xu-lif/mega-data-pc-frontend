...
import { typesMsg } from '../utils';
  ...
  // const getNodeLabe = (nodeVal) => {
  //   const grid_info = sourceData.grid_layout || {}
  //   if (nodeVal.type === 'gen') { // 发电单元
  //     return `有功功率：${(grid_info.gen_info || {}).gen_p}`
  //   } else if (nodeVal.type === 'load') { // 负载
  //     return `有功功率：${(grid_info.load_info || {}).load_p}`
  //   } else {
  //     return `${(typesMsg[nodeVal.type] || {}).name}_${nodeVal.id}`
  //   }
  // }
  ...
  const getOptions = (lines = [], nodes = {}, gridInfo, gridLayout) => {
    const echartsData = Object.keys(nodes).map(key => {
      const val = nodes[key]
      // const labelName = getNodeLabe(val)
      let pValue = ''
      if (val.type === 'load') {
        pValue = gridInfo.load_info.load_p[val.id]
      } else if (val.type === 'gen') {
        pValue = gridInfo.gen_info.gen_p[val.id]
      }
        return {
          name: `${(typesMsg[val.type] || {}).name}_${parseInt(val.id) + 1}`,
          symbol: val.type === 'sub' ? 
    ...
    if (lines.length) {
      lines.forEach((val, index) => {
        ...
        echartLines.push(
          {
            source: `${(typesMsg[val.source.type] || {}).name}_${parseInt(val.source.id) + 1}`,
            target: `${(typesMsg[val.target.type] || {}).name}_${parseInt(val.target.id) + 1}`,
            lineStyle: {
              normal: {
                color: currentColor,
                opacity: lineV > 0 ? 1 : 0
        ...
        if (val.source.position &&  val.target.position && lineV > 0) {
          trailLiens.push(
            {
              coords: [
                val.source.position,      
...
export default React.memo(CenterMap)
